...
```
target 決定：
  content_should_run == true 且 Content process 活著 → target = Content
  content_should_run == true 但 Content 死了          → target = Lobby
  content_should_run == false                         → target = Lobby

前景 == target？或偵測不到前景？ → 不動
其他 app 搶前景（Meta Menu / Quest Home / 其他）→ 每個前景事件只處理一次：
  ├─ Content alive（target=Content）
  │   └─ ✅ REORDER_TO_FRONT 把 Content 拉回前景（不補 extras、不重建 Content）
  └─ target=Lobby（Content 死了，或 server 已 disconnect）
      └─ REORDER_TO_FRONT 把 Lobby 拉回前景
```

**重要**：Content 死掉／被 force-stop／crash 時，`KioskService` **絕對不直接 launch Content**。它把 Lobby 拉回前景，由 `MainActivity.onResume()` 走既有恢復流程（重連 WebSocket → `headset_check_reconnect` 問 server），**只有 server 回 `connect` 才會重新啟動 Content**。這也保證開機時就算 SharedPreferences 還留著舊的 `content_should_run=true`，也會停在 Lobby 而不是直接跳 Content。
...
| `relaunchContent` | 重啟 Content（用於 Lobby 重啟時 Content 仍存活的恢復） |
...
> Watchdog 邏輯已移至 [`KioskService.java`](app/src/main/java/com/quest/lobby/KioskService.java)（`doCheck` / `bringContentToFront`）。`MainActivity` 舊有的 `kioskCheck` / `startKioskWatchdog` / `isContentRunning` 已移除。

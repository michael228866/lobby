...
watchdog 會直接呼叫 `bringContentToFront()`。

這條路徑**不**檢查 `contentAlive`、也**不**依賴 `pidof`，會使用：

```text
NEW_TASK | REORDER_TO_FRONT | SINGLE_TOP
```

並重新附加保存在 `content_extras_json` 的 Intent extras（`restoreExtras`）。

若原本 Content task 還存在，Android 通常會將既有 task 拉回前景；若 task 已被系統移除，Android 可能重新建立 Content Activity。因此不能寫成「一定不重新啟動」或「不重帶 extras」。

這條路徑不依賴 `pidof` 是刻意的：Quest Android 14 可能禁止 Lobby 查詢其他 App PID，造成 `dead (no pid found)`，因此 PID 結果不能用來判定 Meta Menu 立即拉回。

#### 一般前景目標判斷

一般 foreground target 判斷才會計算 `contentAlive`（`shouldRun && isAliveCached(contentPkg)`）。只有以下條件全部成立時，Content 才會成為目標：
...
否則 watchdog 會將 Lobby 拉回前景，交由 Lobby 的 `headset_check_reconnect` 流程向 Server 確認是否需要重新啟動 Content。
...
- Meta／其他 App 立即拉回路徑**不先**檢查 `contentAlive`：只要 session 仍確認且最近前景不是 Lobby 也不是 Content，watchdog 就送出帶有保存 extras 的 `REORDER_TO_FRONT` Intent（既有 task 存在時拉回，task 已被移除時 Android 可能重建 Content Activity）。
- 只有一般 foreground target 判斷才會使用 `contentAlive`；Content 未存活時它不會成為目標，watchdog 改把 Lobby 拉回前景。
- 若最終回到 Lobby，Lobby 透過週期性 idle poll（`headset_check_reconnect`）向 Server 確認遊戲狀態，由 Server 決定是否重新啟動 Content。
...
Lock task allowlist updated
  或（非 Device Owner）Lock task allowlist: NOT device owner
...
`launchContent()` 一進入就先印 `Try launch apk`，通過 duplicate guard 與安裝檢查後才呼叫 `updateLockTaskAllowlist()`，因此 `Try launch apk` 在 `Lock task allowlist ...` 之前。`Lock task allowlist updated` 只有 Device Owner 模式出現；非 Device Owner 會看到 `Lock task allowlist: NOT device owner`。
